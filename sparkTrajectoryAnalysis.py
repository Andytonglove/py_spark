...
def calc_acceleration(speed1, speed2, dtime):
...
def calAcceleration(df):
    ...
    df = df.\
        ...
        calc_acceleration(fun.lag("speed", 1).over(my_window),df.speed,df.dtime)).\
        otherwise(0))
...
if __name__ == "__main__":
    ...
    map(lambda x: {"latitude":float(x[0]), "longtitude":float(x[1]),\
        "date":f"{x[5]} {x[6]}"}).map(lambda p: Row(**(p))).toDF()
