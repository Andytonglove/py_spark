...
'''
这里做了两种验证，第一个是验证是不是有数据为空。
为?的数据视为不空。然后把空行去掉，做第二步验证，就是拿一串很长很长的if。
我验证了每一位的数据是否有效，为数字的进行一个数字验证
（即，验证字符串是否是纯数字），为字符串的在字典里找有没有对应的key，
发现adult.data以及adult.test都没有错误（当然，.test文件的50K.改过来了）。
'''
